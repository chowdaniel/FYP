...
def build_model(input_dim):
    ...
    #15-10 neurons for 2 hidden layers
...
def RP(nLow,nHigh):
    ...
    for i in range(nFits):
        model = build_model(nLow+nHigh)

        opt = Adam(lr=0.001)
        model.compile(optimizer=opt,loss="mse",metrics=["accuracy"])
        
        #Use earlystopping regularization
    ...
    prediction["Replication"] = numpy.mean(prediction,axis=1)
    prediction["SP500"] = valY
    prediction["Diff"] = prediction["Replication"] - prediction["SP500"]
    prediction.to_csv("Network_Prediction.csv")    

    SSE = numpy.sum(numpy.square(prediction["Diff"]))
    
    print "Network SSE: %f" % SSE
